...
import { aiChat, aiChatJSON, PAGE_CONTEXTS as aiPageContexts } from "./ai-service";
...
export async function registerRoutes(
  ...
  app.post("/api/projects/:id/verify-tracking", requireAuth, async (req, res) => {
    try {
      ...
      try {
        ...
        const hasDataProjectId = html.includes(`data-projectId`) || html.includes(`data-project-id`);
        ...
        res.json({
          ...
          checkedUrl: url,
          message: verified
            ? "Tracking code successfully detected on your website!"
            : hasSnippetJs
              ? "Found snippet.js but could not find your project ID. Make sure you're using the correct tracking code."
              : "Tracking code not found on the page. Make sure you've added the snippet to your website's <head> tag.",
        });
      } catch (fetchErr: any) {
        clearTimeout(timeout);
  ...
  app.post("/api/events", async (req, res) => {
    ...
    try {
      ...
      const geoData = shouldAnonymizeIp
  ...
  app.get("/api/projects/:id/traffic-sources", async (req, res) => {
    try {
      ...
      const { from, to } = parseDateRange(req.query);
      const evts = await storage.getEventsByDateRange(req.params.id, from, to);
      const channelMap = new Map<string, { users: Set<string>; sessions: Set<string>; events: number; pageViews: number; bounces: number }>();
      const sourceMap = new Map<string, { users: Set<string>; sessions: Set<string>; events: number }>();
      const campaignMap = new Map<string, { users: Set<string>; sessions: Set<string>; events: number }>();
      ...
      evts.forEach(e => {
        ...
        if (!channelMap.has(channel)) channelMap.set(channel, { users: new Set(), sessions: new Set(), events: 0, pageViews: 0, bounces: 0 });
        const c = channelMap.get(channel)!;
        c.users.add(e.visitorId || e.id);
        const sid = e.sessionId || e.id;
        if (!c.sessions.has(sid) && bounceSessions.has(sid)) c.bounces++;
        c.sessions.add(sid);
        c.events++;
        if (e.eventType === "pageview") c.pageViews++;
        ...
        if (!sourceMap.has(src)) sourceMap.set(src, { users: new Set(), sessions: new Set(), events: 0 });
        const sm = sourceMap.get(src)!;
        sm.users.add(e.visitorId || e.id);
        sm.sessions.add(e.sessionId || e.id);
        sm.events++;

        const meta = e.metadata as any;
        if (meta && (meta.utm_campaign || meta.utmCampaign)) {
          const camp = meta.utm_campaign || meta.utmCampaign || "unknown";
          if (!campaignMap.has(camp)) campaignMap.set(camp, { users: new Set(), sessions: new Set(), events: 0 });
          const cm = campaignMap.get(camp)!;
      ...
      const channels = Array.from(channelMap.entries())
        .map(([name, d]) => ({ name, users: d.users.size, sessions: d.sessions.size, events: d.events, pageViews: d.pageViews, bounceRate: d.sessions.size > 0 ? Math.round((d.bounces / d.sessions.size) * 100) : 0 }))
        .sort((a, b) => b.users - a.users);
      const sources = Array.from(sourceMap.entries())
      ...
      res.json({ period, channels, sources, campaigns });
    } catch (err: any) {
      res.status(500).json({ message: err.message });
    }
  });
  ...
  app.get("/api/ai/status", async (_req, res) => {
    const available = !!process.env.AI_INTEGRATIONS_OPENAI_API_KEY;
    res.json({ available });
  });
  ...
  app.post("/api/ai/chat", requireAuth, async (req, res) => {
    try {
      ...
      const response = await aiChat(systemPrompt, prompt);
      res.json({ response });
    } catch (err: any) {
      res.status(500).json({ message: err.message });
    }
...
Domain: ${project.domain}. Create realistic steps that match the website type. Include 3-6 steps.`;
      ...
      const funnel = await storage.createFunnel({
        projectId: req.params.id,
        name: result.name || "AI-Generated Funnel",
...
Analytics context: ${JSON.stringify(analytics)}`;
      ...
      const analysis = await storage.createPredictiveAnalysis({
        projectId: req.params.id,
        churnRiskScore: result.churnRiskScore || 0,
        churnDrivers: result.churnDrivers || [],
        revenueTrend: result.revenueTrend || { current: 0, projected: [] },
        conversionProbability: result.conversionProbability || 0,
...
Analytics context: ${JSON.stringify(analytics)}`;
      ...
      const audit = await storage.createUxAudit({
        ...
        slowPages: result.slowPages || [],
        flowIssues: result.flowIssues || [],
        navigationIssues: result.navigationIssues || [],
        recommendations: result.recommendations || [],
        summary: result.summary || null,
      });
...
Analytics context: ${JSON.stringify(analytics)}`;
      ...
      const session = await storage.createMarketingCopilotSession({
        projectId: req.params.id,
        prompt: prompt || null,
        seoFixes: result.seoFixes || [],
        ppcOptimizations: result.ppcOptimizations || [],
        uxImprovements: result.uxImprovements || [],
        summary: result.summary || null,
      });
...
Give 3-5 key insights with specific recommendations. Format with markdown.`;
      ...
      const response = await aiChat(systemPrompt, req.body.prompt || "Provide key insights and recommendations for this report data.");
